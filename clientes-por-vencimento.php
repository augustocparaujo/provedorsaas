    ...
    <section class="content">  
      ...
      <form method="post" action="clientes-por-vencimento.php">  
          ...
          <div class="col-lg-2 col-md-2 col-sm-12 vencimento">Dia de vencimento
            ...
            if(@$_POST['vencimento'] != ''){echo '<option value="'.@$_POST['vencimento'].'">'.@$_POST['vencimento'].'</option>';}else{echo'<option value="">selecione</option>';}
            $sqlV = mysqli_query($conexao,"SELECT vencimento FROM cliente WHERE idempresa='$idempresa' GROUP BY vencimento ASC");
            while($r = mysqli_fetch_array($sqlV)){ echo '<option value="'.$r['vencimento'].'">'.$r['vencimento'].'</option>';}
            echo'
            </select>
          </div>
            ...
            <div class="table-responsive no-padding">
              <table class="table table-hover" style="width:100%">
                  ...
                  <tbody>';
                    ...
                    if($_POST['situacao'] != ''){ $situacao = $_POST['situacao']; } else { $situacao = 'Ativo';}

                    $query = mysqli_query($conexao,"SELECT cliente.*, contratos.login AS logincontrato FROM cliente 
                    LEFT JOIN contratos ON cliente.id = contratos.idcliente
                    WHERE cliente.idempresa='$idempresa' AND cliente.vencimento='$vencimento' AND cliente.situacao='$situacao' ORDER BY cliente.nome ASC") 
    ...
    function ExcluirCliente(){
      ...
      $.ajax({
        ...
        success:function(data){
          $('#processando').modal('hide');
          $('#retorno').show().fadeOut(6000).html(data);
          window.setTimeout(function() { history.go(); }, 6001);
