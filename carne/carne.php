...
if (!$ret['rua']) { $endereco = ""; } else { $endereco = AspasForm($ret['rua'].','.$ret['numero'].' - '.$ret['bairro'].' - '.$ret['municipio'].'/'.$ret['estado'].'<br>&emsp;&emsp;'.$ret['cep']); }
if ($ret['cpf'] != '') { $doccliente = $ret['cpf']; } else {  $doccliente = $ret['cnpj'];   }
if (!$ret['valor']) { $valor = ""; } else { $valor = Real($ret['valor']); }
if (!$ret['parcela']) { $parcela = ""; } else { $parcela = $ret['parcela']; }
